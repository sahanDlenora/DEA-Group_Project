...
public class EditProfileServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            login.setPhone(userPhone);
            
            //update database
            LoginDao logindao=new LoginDao();
            boolean ans=logindao.UpdateProfile(login);
            if (ans)
            {
                HttpSession ses = request.getSession();
                ses.setAttribute("update_msg","Update Successfully");
                response.sendRedirect("after_login/edit_profile.jsp");
                
            }
            else
            {
                HttpSession ses = request.getSession();
                ses.setAttribute("update_error_msg","Somthing Went Wrong Server");
                response.sendRedirect("after_login/edit_profile.jsp");
